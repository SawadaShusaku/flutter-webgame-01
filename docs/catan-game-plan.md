...
### 初期配置フェーズ
1. **1巡目**：各プレイヤーが集落1つ + 道路1本を配置（順番通り）
2. **2巡目**：各プレイヤーが集落1つ + 道路1本を配置（逆順）
3. 2巡目の集落周辺の資源を初期資源として獲得

### 配置ルール
- 集落は交差点（3つのタイルが接する点）に配置
- 集落は他の集落から2本分以上離す必要がある
- 道路は辺（2つのタイルの境界）に配置
- 道路は自分の集落/都市から連続している必要がある
...
class Player {
  ...
  // 建設物カウント
  int settlementsBuilt;   // 集落の数（最大5）
  int citiesBuilt;        // 都市の数（最大4）
  int roadsBuilt;         // 道路の数（最大15）
